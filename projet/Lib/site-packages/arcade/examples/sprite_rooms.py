...
class GameView(arcade.View):
    ...
    def on_update(self, delta_time):
        ...
        # to a different room.
        if self.player_sprite.centre_x_in_map > WINDOW_WIDTH and self.current_room == 0:
            self.current_room = 1
            self.physics_engine = arcade.PhysicsEngineSimple(
                self.player_sprite,
                self.rooms[self.current_room].wall_list,
            )
            self.player_sprite.centre_x_in_map = 0
        elif self.player_sprite.centre_x_in_map < 0 and self.current_room == 1:
            self.current_room = 0
            self.physics_engine = arcade.PhysicsEngineSimple(
                self.player_sprite,
                self.rooms[self.current_room].wall_list,
            )
            self.player_sprite.centre_x_in_map = WINDOW_WIDTH
