...
class GameView(arcade.View):
    ...
    def on_update(self, delta_time):
        ...
        if self.player.centre_x < 0 and self.player.change_x < 0:
            self.player.change_x *= -1
        if self.player.centre_y < 0 and self.player.change_y < 0:
            self.player.change_y *= -1

        if self.player.centre_x > WINDOW_WIDTH and self.player.change_x > 0:
            self.player.change_x *= -1
        if self.player.centre_y > WINDOW_HEIGHT and self.player.change_y > 0:
            self.player.change_y *= -1
